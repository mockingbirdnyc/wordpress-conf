...
class WPlookflickr extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
	 		'WPlookflickr',
			__( 'WPlook Flickr', 'conference-wpl' ),
			array( 'description' => __( 'A widget for displaying Flickr images', 'conference-wpl' ), )
		);
	}
	...
	function form($instance) {
		if ( $instance ) {
			...
			$title = __( 'My Flickr', 'conference-wpl' );
		} 

		if ( $instance ) {
			$flickrrss = esc_attr( $instance[ 'flickrrss' ] );
		} else {
			$flickrrss = __( 'insert flickr RSS', 'conference-wpl' );
		} 

		if ( $instance ) {
			$thumbs = esc_attr( $instance[ 'thumbs' ] );
		} else {
			$thumbs = __( '6', 'conference-wpl' );
		} ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id('flickrrss'); ?>">
				<?php _e('Flickr RSS:', 'conference-wpl'); ?>
			</label>
		...
		<p style="font-size: 10px; color: #999; margin: -10px 0 0 0px; padding: 0px;">
			<?php _e('Insert your Flickr RSS.', 'conference-wpl'); ?>
		</p>
		</p>
		<!-- Number of thumbs -->
		<p>
			<label for="<?php echo $this->get_field_id('thumbs'); ?>">
				<?php _e('Number of thumbs:', 'conference-wpl'); ?>
			</label>
		...
		<p style="font-size: 10px; color: #999; margin: -10px 0 0 0px; padding: 0px;">
			<?php _e('Number of thumbnails you want to display.', 'conference-wpl'); ?>
		</p>
		</p>
